...
def calculate():
    ...
    if mode == 1 or mode == 2:
        ...
        if len(x_values) != required or len(y_values) != required:
            messagebox.showerror(
                "Błąd", f"W trybie 1/2 wymagane {required} wartości x i y"
            )
            return
    elif mode == 3:
        required = 2 * nodes
        if len(x_values) != required or len(y_values) != required:
            messagebox.showerror(
                "Błąd", f"W trybie 3 wymagane {required} wartości x i y"
            )
            return
    else:
    ...
    try:
        ...
        messagebox.showerror(
            "Błąd", "Nieprawidłowy format punktu xx dla wybranego trybu"
        )
        return
...
def show_info():
    ...
    - Wynik: Wartość zmiennoprzecinkowa

    Tryb 2 (Przedziały dane rzeczywiste):
    - x: pojedyncze wartości (np. 1 2 3)
    - y: pojedyncze wartości (np. 2 3 5)
    - xx: pojedyncza wartość (np. 2.5)
    - Wynik: Wartość przedziałowa

    Tryb 3 (Przedziały dane przedziałowe):
...
root = tk.Tk()
root.title(
    "Obliczanie wartości i współczynników naturlnej funkcji sklejenia stopnia trzeciego"
)
...
style.configure(
    "TLabel", background=BG_COLOR, font=(FONT_NAME, 10), foreground=TEXT_COLOR
)
...
modes = [
    ...
    ("Tryb 2 - Przedziały dane rzeczywiste", 2),
    ("Tryb 3 - Przedziały dane przedziałowe", 3),
]

for text, val in modes:
    ttk.Radiobutton(
        mode_frame, text=text, variable=mode_var, value=val, style="Toolbutton"
...
def create_input_row(frame, label, row):
    tk.Label(
        frame, text=label, font=(FONT_NAME, 10, "bold"), bg=BG_COLOR, fg=TEXT_COLOR
...
ttk.Button(button_frame, text="Oblicz", command=calculate, style="TButton").pack(
    side="left", padx=10
)
ttk.Button(button_frame, text="ℹ️ Instrukcja", command=show_info, style="TButton").pack(
    side="left", padx=10
)
...
tk.Label(
    footer_frame,
    text="Kalkulator funkcji sklejenia",
    font=(FONT_NAME, 8),
    fg="#7f8fa6",
    bg=BG_COLOR,
).pack()
