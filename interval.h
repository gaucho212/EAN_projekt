 ...
 */
 ...
 enum IAPrecision {
     LONGDOUBLE_PREC = 63, DOUBLE_PREC = 53, FLOAT_PREC = 32, MPREAL_PREC = 128
 };
 ...
 template<typename T> Interval<T> ParseInterval(const string &sa);
 ...
 template<typename T>
 inline Interval<T> ParseInterval(const std::string& sa) {
     // Sprawdź, czy ciąg zaczyna się od '[' i kończy na ']'
     if (sa.front() != '[' || sa.back() != ']') {
         throw std::invalid_argument("Niepoprawny format przedziału. Oczekiwano formatu [a, b].");
     }
 
     // Znajdź pozycję przecinka
     size_t commaPos = sa.find(',');
     if (commaPos == std::string::npos) {
         throw std::invalid_argument("Niepoprawny format przedziału. Brak przecinka.");
     }
 
     // Wyodrębnij lewą i prawą wartość
     std::string leftStr = sa.substr(1, commaPos - 1); // Od znaku po '[' do przecinka
     std::string rightStr = sa.substr(commaPos + 1, sa.size() - commaPos - 2); // Od znaku po przecinku do ']'
     ...
     // Konwertuj lewą i prawą wartość na typ T
     T le = IntRead<T>(leftStr).a; // Użyj IntRead do odczytu lewej wartości
     T re = IntRead<T>(rightStr).b; // Użyj IntRead do odczytu prawej wartości
 ...
 template<>
 inline Interval<mpreal> ParseInterval<mpreal>(const std::string& sa) {
     // Sprawdź, czy ciąg zaczyna się od '[' i kończy na ']'
     if (sa.front() != '[' || sa.back() != ']') {
         throw std::invalid_argument("Niepoprawny format przedziału. Oczekiwano formatu [a, b].");
     }
     ...
     // Konwertuj lewą i prawą wartość na mpreal
     mpreal le = IntRead<mpreal>(leftStr).a; // Użyj IntRead do odczytu lewej wartości
     mpreal re = IntRead<mpreal>(rightStr).b; // Użyj IntRead do odczytu prawej wartości
